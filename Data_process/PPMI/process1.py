...
批量将分层 DICOM (.dcm) 数据集转换为 NIfTI（.nii），并生成 CSV 索引。
更新：
- 同一“时间”目录下的所有 I* 序列全部处理（不再只取第一个）
- CSV 增加一列 test_name（I* 子目录名）
- 生成的 NIfTI 文件名前缀包含 test_name，避免冲突
"""
...
def discover_tasks(root: Path, overwrite: bool, gz: bool) -> Tuple[List[TimeTask], int, int]:
    """
    生成每个 I* 序列的任务；当某时间目录下没有 I* 或 I* 下没有 .dcm 时跳过并告警
    """
    tasks: List[TimeTask] = []
    skipped = 0
    warns = 0
    ...
    for patient_dir in sorted([p for p in root.iterdir() if p.is_dir()]):
        ...
        for modality_dir in sorted([p for p in patient_dir.iterdir() if p.is_dir()]):
            ...
            for time_dir in sorted([p for p in modality_dir.iterdir() if p.is_dir()]):
                ...
                # 对每个 I* 目录都创建任务
                for series_dir in sorted(i_dirs):
                    ...
                    # 不覆盖时：若该前缀对应的 .nii / .nii.gz 已存在则跳过创建任务（但仍会写入 CSV，见 main 中的收集逻辑）
                    test_name = series_dir.name
                    prefix = sanitize(f"{patient_id}_{modality}_{time_name}_{test_name}")
                    if not overwrite:
                        patt = prefix + ("*.nii.gz" if gz else "*.nii")
                        existing = list(time_dir.glob(patt))
                        if existing:
                            continue

                    tasks.append(TimeTask(patient_id, modality, time_name, time_dir, series_dir, test_name))
...
def main():
    ...
    parser.add_argument("--root", type=Path, default='/ailab/group/medai-share/syDu/Brain_EC/PPMI/PPMI', help="数据集根目录")
    parser.add_argument("--csv", type=Path, required=False, default=Path("dataset_index.csv"), help="输出 CSV 路径（默认：./dataset_index.csv）")
    parser.add_argument("--workers", type=int, default=24, help="并行进程数（默认=CPU核数）")
    ...
    tasks, skipped0, warns0 = discover_tasks(args.root, overwrite=args.overwrite, gz=args.gz)
    logging.info("发现任务 %d 个，初始跳过 %d 个，警告 %d 个", len(tasks), skipped0, warns0)

    results = []  # (patient_id, modality, time, test_name, nifti_path)
    total_success = 0
    warns = warns0
    skipped = skipped0

    # 不覆盖模式下：先把已经存在、且能解析出 test_name 的 NIfTI 也纳入 CSV
    if not args.overwrite:
        for patient_dir in sorted([p for p in args.root.iterdir() if p.is_dir()]):
            ...
            for modality_dir in sorted([p for p in patient_dir.iterdir() if p.is_dir()]):
                ...
                for time_dir in sorted([p for p in modality_dir.iterdir() if p.is_dir()]):
                    tname = time_dir.name
                    # 匹配前缀 {pid}_{mod}_{tname}_{test_name}*.nii{.gz}
                    base_prefix = sanitize(f"{pid}_{mod}_{tname}_")
                    glob_patt = base_prefix + ("*.nii.gz" if args.gz else "*.nii")
                    for nii in sorted(time_dir.glob(glob_patt)):
                        fname = nii.name
                        # 提取 test_name（base_prefix 之后到 .nii/.nii.gz 之前的部分，允许额外由 dcm2niix添加的后缀）
                        # 例如：PID_MOD_TIME_TESTNAME_something.nii -> 取第一个 '_' 前为 TESTNAME
                        tail = fname[len(base_prefix):]
                        if tail.endswith(".nii.gz"):
                            tail = tail[:-7]
                        elif tail.endswith(".nii"):
                            tail = tail[:-4]
                        # dcm2niix 可能追加更多信息（如序号）。我们把 test_name 取为第一段
                        test_name = tail.split('_')[0] if '_' in tail else tail
                        results.append((pid, mod, tname, test_name, str(nii.resolve())))
    ...
    if tasks:
        with ProcessPoolExecutor(max_workers=args.workers) as ex:
            ...
            for fut in as_completed(futures):
                ...
                if ok and nii_path:
                    results.append((task.patient_id, task.modality, task.time_name, task.test_name, nii_path))
                    total_success += 1
                    if msg:
                        logging.warning("(%s/%s/%s/%s) %s",
                                        task.patient_id, task.modality, task.time_name, task.test_name, msg)
                        warns += 1
                else:
                    skipped += 1
                    warns += 1
                    logging.warning("转换失败：(%s/%s/%s/%s) %s",
                                    task.patient_id, task.modality, task.time_name, task.test_name, msg or "")
    ...
    with args.csv.open("w", newline="", encoding="utf-8") as f:
        ...
        for row in sorted(results, key=lambda r: (r[0], r[1], r[2], r[3], r[4])):
            writer.writerow(row)
