...
pair MPRAGE and Resting_State_fMRI from the same patient on the same date,
run the same preprocessing pipeline as the original script for each rs-fMRI,
and export atlas BOLD CSV files.

Output naming: patient_id_time.csv
The time value comes from the original Resting_State_fMRI time field,
with unsafe filename characters sanitized (for example, spaces and colons
...
def sh(
    ...
    """Run a command and append stdout/stderr to the log file. Raise on failure."""
    cmd_str = " ".join(map(str, cmd))
    if log_file is None:
        raise RuntimeError("log_file must not be None")
...
def sanitize_filename(s: str) -> str:
    ...
    Keep alphanumeric characters and replace others with '-'.
...
class PairJob:
    """
    Represent one processing task:
    a same-patient rs-fMRI paired with a same-day T1 scan.
    """
    def __init__(
        self,
        patient_id: str,
    ...
    def pair_id(self) -> str:
        """Unique ID for cache directory names, logs, and bookkeeping."""
        return f"{self.patient_id}_{sanitize_filename(self.fmri_time)}"


# --------------------- Single-pair processing pipeline ---------------------
def process_pair(
    job: PairJob,
    outdir: Path,
    atlas_script: Path,
...
def load_pairs_from_csv(csv_path: Path) -> List[PairJob]:
    """
    Read the CSV file and build pair jobs using the following rules:
    - same patient_id
    - same date extracted from the time field
    - modality includes 'MPRAGE' for T1 and 'Resting_State_fMRI' for fMRI

    If multiple fMRI scans exist on the same day, create one job for each fMRI.
    If multiple T1 scans exist on the same day, choose the T1 whose time is closest
    to the fMRI time.
    """
    ...
    for pid, items in by_patient.items():
        ...
        for f in fmri_list:
            ...
            def time_to_seconds(x: str) -> int:
                ...
                Try to parse:
                    YYYY-MM-DD_HH_MM_SS...
                If parsing fails, return a neutral midday value.
                """
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Batch fMRI-to-CSV processing from CSV pairing with FSL, multiprocessing, resume support, and progress reporting."
    )
    parser.add_argument(
        "--csv",
        type=Path,
        required=True,
        help="Input CSV file with columns: patient_id, modality, time, nifti_path"
    )
    parser.add_argument(
        "--outdir",
        type=Path,
        required=True,
        help="Output directory for <patient_id>_<time>.csv and logs"
    )
    parser.add_argument(
        "--atlas-script",
        type=Path,
        default=Path("Data_process/last_process_AAL.py"),
        help="Atlas BOLD extraction script"
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    parser.add_argument(
        "--keep-work",
        action="store_true",
        help="Keep intermediate files in the working directory"
    )
    parser.add_argument(
        "--cache-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=str,
        default="PID({sid})",
        help="Cache subdirectory naming pattern, for example: PID({sid})"
    )
    ...
    if not fsl_dir:
        print(
            "[WARN] $FSLDIR is not set. If later steps fail, please export FSLDIR and add $FSLDIR/bin to PATH."
        )
    ...
    print(f"Reading and pairing from: {args.csv}")
    pair_jobs = load_pairs_from_csv(args.csv)

    if not pair_jobs:
        print(
            "[FATAL] No valid (MPRAGE, Resting_State_fMRI) pairs were found. Please check the CSV content and field names.",
            file=sys.stderr
        )
        sys.exit(1)
