...
# ======================
HYPERPARAMS = {
    'batch_size': 8,                 # 深度学习模型使用
    'learning_rate': 0.0005,         # 深度学习模型使用
    'num_epochs': 100,               # 深度学习模型使用
    'hidden_dim': 256,               # 深度学习模型使用
    'num_blocks': 4,                 # 深度学习模型使用
    'dropout': 0.3,                  # 深度学习模型使用
    'feature_type': 'ALL',           # 'FC', 'UPDRS', or 'ALL'
    'improvement_threshold': 0.25,   # 改善率阈值
    'model_type': 'GCN',             # 'LDA' / 'SVM' / 'TRANSFORMER' / 'GCN'
    'svm_params': {                  # SVM specific parameters
        'C': 1.0,
        'kernel': 'rbf',
        'gamma': 'scale',
        'probability': True
...
def prepare_data(samples):
    """
    准备特征和标签 (用于传统模型 / 也用于K折划分)
    - FC: 使用 166x166 的行均值 -> 166维
    - UPDRS: 33维左右（依CLINICAL_COLS）
    - ALL: 拼接
    """
    X_fc_list = []
    X_updrs_list = []
    ...
    for sample in samples:
        ...
        if feature_type == 'FC':
            ...
            features = np.concatenate([fc_avg, clinical_vec])
        else:
...
class PatientDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            'id': m['id'],
            'fc': fc,  # None if feature_type == UPDRS
            'clinical': clinical,
            'label': y
        }
...
class TransformerClassifier(nn.Module):
    """
    - feature_type == FC:
        tokens = FC matrix rows: (B, 166, 166)
    - feature_type == UPDRS:
        tokens = clinical as sequence: (B, C, 1)
    - feature_type == ALL:
        tokens = FC rows; clinical与pool后的embedding拼接再分类
    """
    def __init__(self, feature_type, clinical_dim, params, dropout=0.3):
        ...
        nhead = params['nhead']
        dim_ff = params['dim_feedforward']
        num_layers = params['num_layers']
        use_cls = params.get('use_cls_token', True)

        self.use_cls = use_cls

        if self.feature_type == 'FC':
            in_dim = 166
            self.token_proj = nn.Linear(in_dim, d_model)
            seq_len = 166 + (1 if use_cls else 0)
        elif self.feature_type == 'UPDRS':
            in_dim = 1
            self.token_proj = nn.Linear(in_dim, d_model)
            seq_len = clinical_dim + (1 if use_cls else 0)
            ...
            raise ValueError(f"Invalid feature_type for Transformer: {self.feature_type}")

        self.pos_enc = PositionalEncoding(d_model=d_model, max_len=max(seq_len, 256), dropout=dropout)
        ...
        if self.feature_type == 'ALL':
            out_dim = d_model + d_model  # pooled + clinical_proj

        self.head = nn.Sequential(
            nn.Linear(out_dim, HYPERPARAMS['hidden_dim']),
            nn.ReLU(),
            nn.Dropout(dropout),
    ...
    def forward(self, fc, clinical):
        ...
        if self.feature_type == 'ALL':
            c = self.clinical_proj(clinical)  # (B,D)
...
def train_dl_model(model, train_loader, num_epochs, lr):
    ...
    # 处理类别不平衡（pos_weight = n_neg / n_pos）
    all_labels = []
...
def cross_validate_experiment(X, y, meta, experiment_name):
    ...
    logger.info(f"Using model type: {HYPERPARAMS['model_type']}")
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y)):
        ...
        if model_type in ['LDA', 'SVM']:
            ...
            if model_type == 'LDA':
                ...
                svm_params = HYPERPARAMS['svm_params']
                model = SVC(
                    C=svm_params['C'],
            ...
            logger.info(f"Training {model_type} model for fold {fold+1}...")
        ...
        # ======================
        # 深度模型：TRANSFORMER / GCN
        # ======================
        elif model_type in ['TRANSFORMER', 'GCN']:
            ...
            if model_type == 'TRANSFORMER':
                model = TransformerClassifier(
                    ...
                    params=HYPERPARAMS['transformer_params'],
            ...
            raise ValueError(f"Invalid model_type: {model_type}. Choose from 'LDA', 'SVM', 'TRANSFORMER', 'GCN'")
...
def main():
    ...
    if model_type == 'GCN' and feature_type == 'UPDRS':
    ...
    # 3. 准备数据（传统特征矩阵；深度模型也用它来做分层K折）
    logger.info("Preparing feature matrices with FC features...")
    X, y, meta = prepare_data(samples)

    # 4. 交叉验证
    experiment_name = f"Treatment_Response_{feature_type}_{model_type}_Thresh{HYPERPARAMS['improvement_threshold']}"
