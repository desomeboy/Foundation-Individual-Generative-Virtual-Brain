...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("ti_integrated_analysis.log", encoding='utf-8'),
...
# 全局配置
# ======================
FMRI_CSV_DIR = "/ailab/group/medai-share/syDu/ruijin/Final_FMRI2MNI/bnpd_csv"
EXCEL_PATH = "/ailab/user/dusiyuan/code/Brain/EC/ruijin/TI_gt_1_10_filled.xlsx"

# AI模型预测文件配置
AI_MODEL_POOLED_FILE = "/ailab/user/dusiyuan/code/Brain/EC/predict_exp/prognotic_pre/PP_regre/TI_Treatment_Regression_all_all_predictions.csv"


AI_MODEL_FOLD_FILES = ['/ailab/user/dusiyuan/code/Brain/EC/predict_exp/prognotic_pre/PP_regre/TI_Treatment_Regression_all_fold1_predictions.csv', 
...
def clean_check_name(check_name):
    ...
    if pd.isna(check_name):
        return ""
    # 移除括号及其内容
    cleaned = re.sub(r'\([^)]*\)', '', str(check_name))
...
def load_fmri_timeseries(check_name, fmri_dir):
    """
    加载fMRI时间序列数据
    
    参数:
        check_name: Check名称 (如 'stn01', 'gpi02')
        fmri_dir: fMRI CSV文件所在目录
    
    返回:
        timeseries: (n_timepoints, n_rois) numpy数组，如果文件不存在返回None
    """
    csv_pattern = f"{check_name}_ts_combined.csv"
...
def parse_excel_data(excel_path):
    """
    解析TI Excel数据，返回病人级别的记录
    只保留同时有治疗前/后记录、且fMRI文件存在的病人
    """
    ...
    # 检查必要列
    required_cols = ['Check', '总分', '靶点位置', '治疗状态', '序号', '日期', '姓名']
    ...
    for (id_num, date), group in tqdm(groups, desc="Processing patients"):
        ...
        # 检查fMRI文件是否存在
        fmri_csv_pre = os.path.join(FMRI_CSV_DIR, f"{clean_check_pre}_ts_combined.csv")
        
        if not os.path.exists(fmri_csv_pre):
            logger.debug(f"[SKIP] Patient {id_num}_{date}: Missing fMRI file - pre_fmri")
            continue
            
        target_folder = pre_row['靶点位置']
        if pd.isna(target_folder) or not target_folder:
            logger.debug(f"[SKIP] Patient {id_num}_{date}: Unknown target location")
            continue
        
        # 计算改善率，如果为负则归为0
        pre_score = float(pre_row['总分'])
        post_score = float(post_row['总分'])
        
        if pre_score > 0:
            improvement_rate = (pre_score - post_score) / pre_score
            # 如果改善率为负（病情恶化），归为0
            if improvement_rate < 0:
                negative_improvement_count += 1
                logger.debug(f"[INFO] Patient {id_num}_{date}: Negative improvement rate "
                           f"({improvement_rate:.4f}) set to 0.0")
    ...
    if improvement_rates:
        ...
        logger.info(f"  N negative (set to 0): {negative_improvement_count}")
...
def analyze_fc_correlation(patients):
    ...
    """
    logger.info("\n" + "="*80)
    logger.info("FC-UPDRS III Improvement Rate Correlation Analysis (TI)")
    logger.info("="*80)
    ...
    for patient in tqdm(patients, desc="Computing FC"):
        # 加载fMRI时间序列
        timeseries = load_fmri_timeseries(patient['check_name'], FMRI_CSV_DIR)
        
        if timeseries is None:
            logger.warning(f"Could not load fMRI for {patient['check_name']}, skipping")
            continue
        ...
        if any(np.isnan(v) for v in fc_features.values()):
            logger.warning(f"NaN in FC features for {patient['check_name']}, skipping")
            continue
        ...
        all_data.append({
            ...
            'target_original': patient['target_original'],
    ...
    # 保存原始数据
    df.to_csv('ti_fc_improvement_data.csv', index=False)
    logger.info(f"\nSaved raw data to ti_fc_improvement_data.csv (N={len(df)})")
    ...
    # 保存结果
    with open('ti_fc_correlation_results.json', 'w') as f:
        json.dump({
            'n_samples': len(df),
    ...
    logger.info("\nSaved correlation results to ti_fc_correlation_results.json")
...
#     """
#     分析AI模型预测结果（TI数据只有pooled版本）
#     """
#     logger.info("\n" + "="*80)
#     logger.info("AI Model Prediction Analysis (TI)")
#     logger.info("="*80)
...
#         # 计算Spearman相关性
#         if 'true_value' in pooled_data.columns and 'pred_value' in pooled_data.columns:
#             r_spearman, p_spearman = spearmanr(pooled_data['true_value'], 
#                                                pooled_data['pred_value'])
#             r_pearson, p_pearson = pearsonr(pooled_data['true_value'], 
#                                            pooled_data['pred_value'])
            
#             logger.info(f"\nPooled Prediction Results:")
#             logger.info(f"  Spearman R = {r_spearman:.3f}, P = {p_spearman:.4e}")
#             logger.info(f"  Pearson R = {r_pearson:.3f}, P = {p_pearson:.4e}")
#         else:
#             r_spearman, p_spearman = None, None
#             r_pearson, p_pearson = None, None
#             logger.warning("Missing 'true_value' or 'pred_value' columns in pooled data")
#     else:
...
#     # 创建一个2x3的子图布局 (4个FC对 + 1个AI模型 + 1个统计摘要)
...
#         # 散点图
#         ax.scatter(fc_values, improvement_rates, c='#2E86AB', s=80, alpha=0.6,
...
#         # 标题和标签
#         ax.set_xlabel(f'FC {pair_name}', fontsize=12, fontweight='bold')
#         ax.set_ylabel('UPDRS III Improvement Rate', fontsize=12, fontweight='bold')
#         ax.set_title(f'FC {pair_id.upper()}: {pair_name}\n' + 
#                      f'R = {spearman_r:.2f} (P = {spearman_p:.3e}), R² = {r_squared:.3f}',
...
#         # 计算相关性
#         r_spearman = ai_model_results['spearman_r']
#         p_spearman = ai_model_results['spearman_p']
        
#         # 线性回归
#         slope, intercept, r_pearson, p_pearson, se = linregress(x, y)
        
#         # 散点图
#         ax_ai.scatter(x, y, c='#2E86AB', s=80, alpha=0.6,
#                      edgecolors='none', linewidth=1.5, zorder=3)
...
#         # 绘制拟合线
#         ax_ai.plot(x_line, y_line, 'r-', linewidth=2, label='Linear fit', zorder=2)
        
#         # 绘制置信区间
#         ax_ai.fill_between(x_line, ci_lower, ci_upper, 
#                           alpha=0.15, color='red', label='95% CI', zorder=1)
        
#         # 计算R²
#         r_squared = r_pearson ** 2
        
#         # 标题和标签
#         ax_ai.set_xlabel('Model Prediction', fontsize=12, fontweight='bold')
#         ax_ai.set_ylabel('Improvement Rate (empirical)', fontsize=12, fontweight='bold')
#         ax_ai.set_title(f'AI Model Prediction (TI)\n' +
#                        f'R = {r_spearman:.2f} (P = {p_spearman:.3e}), R² = {r_squared:.3f}',
#                        fontsize=12, fontweight='bold')
#         ax_ai.grid(True, alpha=0.3)
#         ax_ai.legend(loc='best', fontsize=9)
#         ax_ai.tick_params(labelsize=10)
...
#     # 创建摘要表格数据
#     summary_data = []
#     summary_data.append(['', 'Spearman R', 'P-value', 'N'])
#     summary_data.append(['=' * 50, '=' * 50, '=' * 50, '=' * 50])
    
#     # FC结果
#     for pair_id in ['pair1', 'pair2', 'pair3', 'pair4']:
#         pair_name = FC_ROI_PAIRS[pair_id]['name']
#         r = fc_correlation_results[pair_id]['spearman_r']
#         p = fc_correlation_results[pair_id]['spearman_p']
#         n = len(fc_df)
#         summary_data.append([f'FC {pair_name}', f'{r:.3f}', f'{p:.3e}', f'{n}'])
    
#     summary_data.append(['─' * 50, '─' * 50, '─' * 50, '─' * 50])
    
#     # AI模型结果
#     if ai_model_results['spearman_r'] is not None:
#         summary_data.append(['AI Model (Pooled)', 
#                            f"{ai_model_results['spearman_r']:.3f}",
#                            f"{ai_model_results['spearman_p']:.3e}",
#                            f"{len(ai_model_results['pooled_data'])}"])
    
#     # 绘制表格
#     table = ax_summary.table(cellText=summary_data, loc='center', 
#                             cellLoc='left', colWidths=[0.35, 0.25, 0.25, 0.15])
...
#     ax_summary.set_title('Summary Statistics (TI)', fontsize=14, fontweight='bold', pad=20)
    
#     # 保存图片
#     plt.savefig('ti_integrated_analysis.png', dpi=300, bbox_inches='tight')
#     plt.savefig('ti_integrated_analysis.pdf', dpi=300, bbox_inches='tight')
#     logger.info("Saved integrated visualization to ti_integrated_analysis.png and .pdf")
#     plt.close()
...
def create_integrated_visualizations(fc_df, fc_correlation_results, ai_model_results):
    ...
    for idx, pair_id in enumerate(['pair1', 'pair2', 'pair3', 'pair4']):
        ...
        # 散点图
        scatter = ax.scatter(fc_values, improvement_rates, c='#6A4C9C', s=40, alpha=1,
        ...
        # 绘制拟合线
        line = ax.plot(x_line, y_pred, "k-", alpha=0.8, linewidth=2.5, label='Linear fit', zorder=2)[0]
    ...
    if ai_model_results['pooled_data'] is not None:
        ...
        # 散点图
        ax_ai.scatter(x, y, c='#6A4C9C', s=40, alpha=1,
                     edgecolors='none', linewidth=1.5, zorder=3)
...
def analyze_by_target(df):
    ...
    """
    logger.info("\n" + "="*80)
    logger.info("Target-Specific Correlation Analysis (TI)")
    logger.info("="*80)
    ...
    # 保存结果
    with open('ti_fc_correlation_by_target.json', 'w') as f:
        json.dump(target_results, f, indent=2)
    
    logger.info("\nSaved target-specific results to ti_fc_correlation_by_target.json")
...
def main():
    logger.info("="*80)
    logger.info("Starting Integrated TI Analysis (FC + AI Model)")
    logger.info("="*80)
    
    # 1. 解析Excel数据
    logger.info("\n[Step 1] Parsing TI Excel data...")
    ...
    logger.info("="*80)
    logger.info("\nGenerated files:")
    logger.info("  - ti_fc_improvement_data.csv          : Raw FC and improvement rate data")
    logger.info("  - ti_fc_correlation_results.json      : FC correlation statistics")
    logger.info("  - ti_fc_correlation_by_target.json    : Target-specific FC correlation")
    logger.info("  - ti_integrated_analysis.png/pdf      : Integrated visualization (FC + AI)")
    logger.info("  - ti_integrated_analysis.log          : Detailed log file")
    logger.info("\n" + "="*80)
