...
# ======================
def parse_args():
    parser = argparse.ArgumentParser(description='Treatment Response Prediction with Seed/Epoch Search (Incremental CSV)')
    ...
    parser.add_argument('--seed_end', type=int, default=999, help='Random seed end (inclusive)')

    # 增量写入控制
    parser.add_argument('--resume', action='store_true', default=True,
                        help='Resume if CSV exists: skip combinations already present (default: True)')
    parser.add_argument('--no_resume', dest='resume', action='store_false',
                        help='Disable resume behavior (overwrite CSV header if exists)')
...
def prepare_response_data(patients, sample_type='all', apply_zscore=False):
    ...
    for patient in tqdm(patients, desc="Loading diff features"):
        ...
        y.append(response)
        meta.append({**patient, 'response_label': response, 'feature_type': sample_type})
...
def cross_validate_auc_ap_only(
    ...
    for _fold, (train_idx, test_idx) in enumerate(skf.split(X, y)):
        ...
        train_loader = DataLoader(DiffDataset(X_train, y_train), batch_size=batch_size, shuffle=True)
        test_loader = DataLoader(DiffDataset(X_test, y_test), batch_size=batch_size, shuffle=False)
...
# 增量写CSV（每个组合写一行）
# ======================
def build_output_columns(n_splits: int):
    cols = ["random_seed", "num_epochs"]
    for i in range(n_splits):
        cols += [f"auc_fold{i+1}", f"ap_fold{i+1}"]
    cols += ["auc_mean", "ap_mean"]
    return cols


def load_done_pairs(csv_path: str):
    """
    读取已完成的 (random_seed, num_epochs) 组合，用于断点续跑。
    CSV 很大时读取会慢一些；但对可靠断点更稳。
    """
    if not os.path.exists(csv_path):
        return set()

    try:
        df = pd.read_csv(csv_path, usecols=["random_seed", "num_epochs"])
        done = set(zip(df["random_seed"].astype(int).tolist(), df["num_epochs"].astype(int).tolist()))
        return done
    except Exception as e:
...
def append_row_to_csv(csv_path: str, row_dict: dict, columns: list):
    """
    以“追加模式”写入一行：不存在则先写header。
    为了尽量防止中途断电导致写坏：写入后 flush+fsync。
    """
    file_exists = os.path.exists(csv_path)
    # 统一列顺序
    row = {c: row_dict.get(c, np.nan) for c in columns}
    df_row = pd.DataFrame([row], columns=columns)

    with open(csv_path, "a", encoding="utf-8", newline="") as f:
        df_row.to_csv(f, index=False, header=(not file_exists))
        f.flush()
        os.fsync(f.fileno())


# ======================
# 搜索 random_seed & num_epochs，并增量保存
# ======================
def search_seed_and_epochs_incremental(X, y, out_csv_path: str):
    columns = build_output_columns(args.n_splits)

    # 断点续跑：跳过已有组合
    done_pairs = load_done_pairs(out_csv_path) if args.resume else set()
    if args.resume and len(done_pairs) > 0:
        logger.info(f"Resume enabled: {len(done_pairs)} combinations already in CSV, will skip them.")
    ...
    logger.info(f"Total combinations (including skipped): {total_runs}")
    ...
    for num_epochs in epoch_list:
        for seed in seeds:
            ...
            if args.resume and (seed, num_epochs) in done_pairs:
                continue
            ...
            row = {"random_seed": seed, "num_epochs": num_epochs}
            ...
            # 立即追加写入CSV
            append_row_to_csv(out_csv_path, row, columns)

            # 更新最佳（按auc_mean优先，其次ap_mean）
            auc_m = row["auc_mean"]
            ap_m = row["ap_mean"]
            if not np.isnan(auc_m):
                if (auc_m > best_auc) or (np.isclose(auc_m, best_auc) and (not np.isnan(ap_m)) and ap_m > best_ap):
                    best_auc = auc_m
                    best_ap = ap_m if not np.isnan(ap_m) else -np.inf
                    best_combo = (seed, num_epochs)

    pbar.close()

    logger.info(f"Saved incremental search results to: {out_csv_path}")
    if best_combo is not None:
        logger.info(f"Best combo so far (by auc_mean then ap_mean): seed={best_combo[0]}, epochs={best_combo[1]}, "
                    f"auc_mean={best_auc:.6f}, ap_mean={(best_ap if best_ap!=-np.inf else float('nan')):.6f}")
...
def main():
    ...
    logger.info(f"Search: epochs={args.epoch_candidates}, seeds=[{args.seed_start}..{args.seed_end}], resume={args.resume}")
    ...
    search_seed_and_epochs_incremental(X, y, out_csv_path=out_csv)
