...
def run_shap(model: nn.Module, X: np.ndarray, out_dir: str):
    ...
    if ft in  ["DIFF","ALL"]:
        ...
        try:
            ...
            # ========== 帕金森 Top30 中 SHAP 前15：合并 beeswarm + bar ==========
# ========== 帕金森 Top30 中 SHAP 前15：cbar(最左) + bee(更宽,左) + yticks(中) + bar(右) ==========
            import inspect

            # ---------- 你可以在这里调样式 ----------
            bar_color = "#8a508f"        # 自定义柱状图颜色（改成你想要的）
            bar_alpha = 0.95
            xlabel_fs = 18              # bar x轴label字体
            bar_tick_fs = 14            # bar x轴刻度字体
            ytick_fs = 16               # 中间yticks字体
            bee_xlabel_fs = 18
            cbar_tick_fs = 14
            cbar_label_fs = 16

            # 画布：建议比你原来更宽一点（bee要更宽 + 右侧还要放bar）
            fig = plt.figure(figsize=(17, 7))

            # 4个区域：cbar(最左) / bee(左) / ytick-label轴(中) / bar(右)
            # [left, bottom, width, height]
            pos_cbar = [0.05, 0.18, 0.018, 0.64]
            pos_bee  = [0.20, 0.08, 0.77, 0.84]   # bee 更宽（你要更宽就增大 width）
            pos_lbl  = [0.87, 0.08, 0.13, 0.84]   # yticks 放中间的“标签轴”
            pos_bar  = [1.1, 0.08, 0.35, 0.84]   # bar 轴
            ...
            # ---------- 2) 从 beeswarm 轴里“抓”到 shap 排序后的 yticks / yticklabels ----------
            ...
            ax_lbl.tick_params(axis='y', length=0, pad=0)           # pad=0 避免偏移
            ax_lbl.yaxis.set_ticks_position('left')                 # tick 还是挂在“左侧逻辑”，但我们手动放位置
            
            for t in ax_lbl.get_yticklabels():
                t.set_horizontalalignment('center')                 # 文本水平居中
                t.set_x(0.5)                                        # 文本锚点放在轴的中间 (x=0.5)

            # 把中间标签轴搞成“纯文字”：去掉边框、刻度线
            for spine in ax_lbl.spines.values():
                spine.set_visible(False)
            ax_lbl.tick_params(axis='y', length=0)          # 不要小刻度线
            ax_lbl.yaxis.set_ticks_position('none')         # 不显示tick marker

            # ---------- 4) 右侧柱状图：自己用 matplotlib 画（颜色可控），并和 beeswarm 的 y 对齐 ----------
            # 先计算每个特征 mean(|SHAP|)
            ...
            # 按“shap.summary_plot 实际显示的顺序”取 bar 的值（用 yticklabels 对齐）
            ...
            # bar 轴不需要 ytick（因为中间已经显示了）
            ax_bar.tick_params(axis='y', left=False, labelleft=False, right=False, labelright=False)
            for spine in ["top", "right", "left"]:
                ax_bar.spines[spine].set_visible(False)

            # 强制 y 范围一致（sharey 通常够，但保险）
            ax_lbl.set_ylim(ax_bee.get_ylim())
            ax_bar.set_ylim(ax_bee.get_ylim())

            # ---------- 5) 保存 ----------
            plt.savefig(
                os.path.join(out_dir, "shap_REGION_ParkinsonTop30_SHAPTop15_cbar_bee_ticks_bar.pdf"),dpi=300, bbox_inches="tight", pad_inches=0.4)
            plt.close(fig)
